...
spec = describe "Db (integration)" $ around withTestPool $ do
  ...
  describe "upsertHead" $ do
    it "inserts a new head" $ \pool -> do
      Db.upsertHead pool "head-1" "localhost" 4001 "Open" False Nothing
    ...
    it "updates existing head on conflict" $ \pool -> do
      Db.upsertHead pool "head-1" "localhost" 4001 "Open" False Nothing
      Db.upsertHead pool "head-1" "localhost" 4001 "Closed" False Nothing
      mHead <- Db.getHead pool "head-1"
      case mHead of
  ...
  describe "getAllHeads" $ do
    it "returns all registered heads" $ \pool -> do
      Db.upsertHead pool "head-1" "localhost" 4001 "Open" False Nothing
      Db.upsertHead pool "head-2" "localhost" 4002 "Closed" False Nothing
  ...
  describe "getAllHeadsPaginated" $ do
    it "returns paginated results" $ \pool -> do
      Db.upsertHead pool "head-1" "localhost" 4001 "Open" False Nothing
      Db.upsertHead pool "head-2" "localhost" 4002 "Open" False Nothing
      Db.upsertHead pool "head-3" "localhost" 4003 "Open" False Nothing
