...
*Recent updates:*
- 2026-08-04 — **Agent docker images on GHCR**: `agent/Dockerfile` (multi-stage: `haskell:9.6.7-slim` build → `debian:bookworm-slim` runtime, non-root, `HYDRA_AGENT_STATE_FILE` defaulted into a `/data` volume so identity survives recreation). `release-agent` workflow now also pushes `ghcr.io/v0d1ch/hydra-registry-agent:{version,latest}` as a multi-arch manifest (amd64 + arm64, built natively per arch); `workflow_dispatch` with a version input builds images for an existing release without re-tagging. Setup page/README/wiki document the `docker run --network host -v hydra-agent-state:/data …` invocation.
